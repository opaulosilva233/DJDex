...
export default function AddFestivalForm({ handleAddFestival }) {
	...
	return (
		...
		>
			<div>
				<h2 className="m-0 text-xl font-semibold text-slate-900 dark:text-gray-100">Adicionar Festival</h2>
				<p className="mt-1 text-sm text-slate-500 dark:text-slate-300">
					Cria a entidade do festival com o nome e a localização.
				</p>
			</div>
			...
			<div className="grid w-full gap-4 lg:grid-cols-2 [grid-template-columns:repeat(auto-fit,minmax(260px,1fr))]">
				...
				<label className="grid gap-1 text-sm font-medium text-slate-700 dark:text-gray-100">
					<span className="text-slate-700 dark:text-gray-100">Local</span>
					<input name="local" value={formData.local} onChange={handleChange} className={inputClassName} required />
				</label>

				<label className="grid gap-1 text-sm font-medium text-slate-700 dark:text-gray-100">
					<span className="text-slate-700 dark:text-gray-100">Ano</span>
					<input name="ano" type="number" value={formData.ano} onChange={handleChange} className={inputClassName} required />
				</label>
